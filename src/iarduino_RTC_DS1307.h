...
class iarduino_RTC_DS1307: public iarduino_RTC_BASE{											//
	...
	//	Инициализация модуля:																	//
		void	begin(iarduino_I2C_Select* i){													//	(указатель на объект работы с выбранной шиной I2C)
					selI2C=i;																	//	Сохраняем адрес указателя на выбранную шину I2C
				...
				//	Установка флагов управления и состояния модуля:								//
					varI=selI2C->readByte(valAddress, 0x00); if( varI & 0b10000000                         ){selI2C->writeByte(valAddress, 0x00, (varI&~0b10000000)            );}	//		(если установлен 7 бит в 0 регистре, то сбрасываем его - запускаем генератор)
					varI=selI2C->readByte(valAddress, 0x02); if( varI & 0b01000000                         ){selI2C->writeByte(valAddress, 0x02, (varI&~0b01000000)            );}	//		(если установлен 6 бит в 2 регистре, то сбрасываем его - переводим модуль в 24 часовой режим)
					varI=selI2C->readByte(valAddress, 0x07); if((varI & 0b00000011) || !(varI & 0b00010000)){selI2C->writeByte(valAddress, 0x07, (varI&~0b00000011)|0b00010000 );}	//		(если установлены 1 и 0 биты или сброшен 4 бит в 7 регистре, то сбрасываем 1 с 0 битами, а 4 устанавливаем - выводим меандр с частотой 1 Гц на выводе SQW/OUT модуля)
		}																						//
																								//
	//	Чтение одного значения из регистров даты и времени модуля:								//
		uint8_t	funcReadTimeIndex(uint8_t i){													//	(i = 0-секунды / 1-минуты / 2-часы / 3-день / 4-месяц / 5-год / 6-день недели)
					delay(1);																	//
					return selI2C->readByte(valAddress, arrTimeRegAddr[i]) & arrTimeRegMack[i];	//
		}																						//
																								//
	//	Запись одного значения в регистры даты и времени модуля:								//
		void	funcWriteTimeIndex(uint8_t i, uint8_t j){										//	(i = 0-секунды / 1-минуты / 2-часы / 3-день / 4-месяц / 5-год / 6-день недели, j = значение)
					...
					selI2C->writeByte(valAddress, arrTimeRegAddr[i], j);							//	Сохраняем значение j в регистр arrTimeRegAddr[i]
		}																						//
																								//
	private:																					//
