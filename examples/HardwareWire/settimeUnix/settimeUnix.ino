...
//   iarduino_RTC watch(RTC_DS1302, 2, 3, 4);               // Создаём объект watch для работы с RTC модулем на базе чипа DS1302, указывая выводы Arduino подключённые к выводам модуля RST, CLK, DAT.
//   iarduino_RTC watch(RTC_DS1307);                        // Создаём объект watch для работы с RTC модулем на базе чипа DS1307, используется шина I2C.
     iarduino_RTC watch(RTC_DS3231);                        // Создаём объект watch для работы с RTC модулем на базе чипа DS3231, используется шина I2C.
//   iarduino_RTC watch(RTC_RX8025);                        // Создаём объект watch для работы с RTC модулем на базе чипа RX8025, используется шина I2C.
                                                            //
void setup(){                                               //
     ...
     watch.settimezone(3);                                  // Указываем свой часовой пояс. Так как время с начала эпохи Unix считается по гринвичу.
     watch.settimeUnix(1577836800);                         // Записываем время в модуль: 01.01.1970 00:00:00 (дата начала эпохи Unix) + 1577836800 сек = 01.01.2020 00:00:00 GMT = 01.01.2020 03:00:00 MSC.
}                                                           //
void loop(){                                                //
