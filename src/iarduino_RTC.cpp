...
void	iarduino_RTC::settime(const char* str){																//	str =	"Mon Oct 02 15:29:23 2023" (24 символа).
			settime(	(str[17]-'0')*10+(str[18]-'0'),														//	сек :	"00"..."59"					=> 0...59
						...
						funcDayMon(&str[4]),																//	мес :	"Jan"..."Dec"				=> 1...12
						(str[22]-'0')*10+(str[23]-'0'),														//	год :	"0000"..."9999"				=> 0...99
						funcDayMon(str) );																	//	д/н :	"Sun"..."Sat"				=> 0...6
}
...
//		Преобразование строки с названием дня или месяца в число:											//	Функция введена в версии 2.0.3 вместо функции memmem() которую не поддерживала плата Arduino UNO R4
int		iarduino_RTC::funcDayMon(const char* a){															//	(строка "Mon" ... "Sun" / "Jan" ... "Dec")
			int i; char* j;																					//	i - номер проверяемого дня или месяца, j - часть строки charDayMon соответствующая названию проверяемого дня или месяца.
			for(i=0;i< 7;i++){j=(char*)&charDayMon[i*3   ]; if(a[0]==j[0]&&a[1]==j[1]&&a[2]==j[2]){return i;   }}	//	Если a[0-2] == j[0-2] то возвращаем номер дня    (0...6).
			for(i=0;i<12;i++){j=(char*)&charDayMon[i*3+21]; if(a[0]==j[0]&&a[1]==j[1]&&a[2]==j[2]){return i+1; }}	//	Если a[0-2] == j[0-2] то возвращаем номер месяца (1...12).
			return 0;																						//	Если совпадений не обнаружено, возвращаем 0.
}
