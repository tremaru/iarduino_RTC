	...
	//	Инициализация модуля:																	//
		void	begin(iarduino_I2C_Select* i){													//	(указатель на объект работы с выбранной шиной I2C)
				...
				//	Установка флагов управления и состояния модуля:								//
					...
					varI=selI2C->readByte(valAddress, 0x0E); if( varI & 0b11011111                         ){selI2C->writeByte(valAddress, 0x0E, (varI&~0b11011111)            );}	//		(если установлены 7,6,4,3,2,1 и 0 биты в 14 регистре, то сбрасываем их - разрешаем генератору работать от батарейки, запрещаем выводу SQW работать от батарейки, выводим меандр с частотой 1Гц на вывод SQW, переводим вывод INT/SQW в режим SQW, запрещаем прерывания будильников)
					varI=selI2C->readByte(valAddress, 0x0F); if((varI & 0b10000011) || !(varI & 0b00001000)){selI2C->writeByte(valAddress, 0x0F, (varI&~0b10000011)|0b00001000 );}	//		(если установлены 7,1 и 0 биты или сброшен 3 бит в 15 регистре, то сбрасываем 7,1 и 0 биты, а 3 устанавливаем - сбрасываем флаг остановки генератора, разрешаем меандр с частотой 32768Гц на выводе 32kHz, сбрасываем флаги будильников)
		}																						//
																								//
